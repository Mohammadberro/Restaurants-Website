...
function Viewrest() {
  let Viewcontainer = document.getElementById("viewrest-container");
  Viewcontainer.innerHTML="";
  let namesArray = JSON.parse(localStorage.getItem("res_name") || "[]");
  for (let i = 0; i <= namesArray.length; i++) {
    names = namesArray[i];
    Viewcontainer.innerHTML += `<div><h2>${names}</h2><button id="${names}"onclick="RemoveRest(this)">remove</button></div>`
  }
}
